 ...
 * 
 * $Id: SimpleHibernateDao.java,v 1.4 2014/03/10 00:41:29 administrator Exp $
 */
...
public class SimpleHibernateDao<T, PK extends Serializable> {
	 ...
	 *	获取全部对象,支持排序.
	 */
	public List<T> getAll(boolean isAsc,String... orderBy) {
	 ...
	 * @param values 数量可变的参数,按顺序绑定.
	 */
	public <X> List<X> findByQueryName(final String queryName, final Object... values) {
		return createQueryByName(queryName, values).list();
	}
	...
	public int[] execBatchSqls(final String[] sqls) {
		
		int rInt[]=new int[sqls.length];
		for(int i=0;i<sqls.length;i++){
		Query query = getSession().createQuery(sqls[i]);
		rInt[i]=query.executeUpdate();
		}
		return rInt;
	}
    public int[] execBatchNativeSqls(final String[] sqls) {
		
		int rInt[]=new int[sqls.length];
		for(int i=0;i<sqls.length;i++){
		Query query = getSession().createSQLQuery(sqls[i]);
		rInt[i]=query.executeUpdate();
		}
		return rInt;
	}
	...
	/**
	 * 根据查询queryName与参数列表创建Query对象.
	 * 
	 * 本类封装的find()函数全部默认返回对象类型为T,当不为T时使用本函数.
	 * 
	 * @param values 数量可变的参数,按顺序绑定.
	 */
	public Query createQueryByName(final String queryName, final Object... values) {
	 ...
	 * @param values 数量可变的参数,按顺序绑定.
	 */
	public Query createQueryByName(final String queryName, final Map<String, ?> values) {
		Assert.hasText(queryName, "queryName不能为空");
		Query query = getSession().getNamedQuery(queryName);
		if (values != null) {
